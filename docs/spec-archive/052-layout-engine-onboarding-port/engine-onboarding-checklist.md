...
5. If this is a new `packages/graph-layout-*` package, add a package lock and
   keep it self-contained.
   - Dependencies should be only the algorithm package plus
     `@diagram-generator/graph-layout-core` unless the engine truly needs more.
   - The package `test` script should build `graph-layout-core` and the package
     before running Vitest, so downstream packages can resolve `dist/`.

## Step 2 — Render adapter (in `packages/layout-engine`)

6. Add a `PreviewFrameDiagramRenderAdapter` that calls `<algo-fn>` and returns a
   layout result.
...
7. If the engine reads namespaced YAML options, thread that namespace through the
   generic frame path instead of adding engine-id branches.
   - Use `FrameDiagram.engineLayout['meta.<engine>']` and pass it to the graph
     adapter as generic option overrides.
   - Add serialization/lowering coverage if this is the first package using a
     new namespace family.

## Step 3 — Control specs

8. Build control specs from the param registry using the generic mapper:
...
9. Confirm the namespace is exposed by the engine's manifest control specs and
   covered by the preview save contract.
   - File: `apps/preview/src/persistence/frame-engine-layout-namespaces.ts`
   - The built-in `meta.*` namespaces are derived from registered preview-engine
     control specs; add supported key assertions and one persistence test so
     unsupported overrides fail before they reach YAML.

## Step 4 — Preview engine definition

10. Create the engine definition file.
   ...
   - Call `defineGraphLayoutPreviewEngine({ ... })` with:
     ...
     - `sidebarSections`: `['elk-layout']` for ELK-family engines, `['graph-layout']`
       for non-ELK graph engines using the generic graph-layout panel, or the
       section your engine exposes (do not invent a new section without a UI
       task)
...
12. Export the engine through the existing barrels, including browser-entry
    runtime namespaces when the browser needs the engine or its control specs.
    - Files normally include `preview-engine/index.ts`, `builtins.ts`,
      `src/index.ts`, `browser-entry-preview-engines.ts`, and
      `browser-entry-flat-preview-engine-barrel.ts`.
    - If this is a new graph-layout package, update
      `packages/layout-engine/build-browser.mjs` so `build:browser` builds its
      `dist/` before bundling.

## Step 6 — Permit the engine key

13. Add `'<engine>'` to the permitted `meta.layout_engine` values.
...
14. Add one contract test.
    - File: `packages/layout-engine/tests/engines/<engine>.contract.test.ts`
    - Body: `runGraphLayoutPreviewEngineContract(<the definition>)`.
    - Verify: `npm --prefix packages/layout-engine test`
15. Keep `packages/layout-engine/tests/no-engine-id-branching.test.ts` green.
    Any failure here means the engine was wired through a central branch instead
    of the registry/factory.

## Step 8 — Host-contract + live proof

16. Add/extend the apps/preview host-contract test asserting this engine shows
    only its own sections and hides the others (grid for non-v3, ELK for
    non-ELK, graph-layout for engines that do not use it, force for non-force).
    - Verify: `npm --prefix apps/preview test`
17. Rebuild the browser bundle and run a no-screenshot DOM probe on a diagram
...
18. Run the full validation set (spec 052 SC-001..SC-005) and confirm the diff
    for this engine touched only: its definition file, its render adapter entry,
    its param/control helper, the single registration line, required export
    barrels, namespace persistence when applicable, the permitted-keys line, and
    its tests. If it touched a central branch, you did it wrong.
