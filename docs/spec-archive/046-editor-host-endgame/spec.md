...
Closeout reconciliation note: the package was reopened correctly while the
many-engine bar was still unsettled, but the later workstreams and closeout
audits did in fact meet that bar. On 2026-07-05 the stale reopen text, stale
line counts, and stale veto bookkeeping were reconciled: the honest answer to
the "50 / 150 / 500 engines" question is now **yes** so long as onboarding
continues through typed registration points rather than widening `editor.js`,
...
But that closeout originally left a second problem unresolved:

- `scripts/preview/editor.js` was still 1,601 physical lines when the reopen
  call was made
...
The current `editor.js` is better than the old monolith, but repeated "500
lines smaller" iterations without an explicit endgame would still only defer
the same problem. The closeout work in this package existed to end that
migration state, not cosmetically reduce it.
...
One residual seam remains, but it does not block closeout: a brand-new shell
family beyond the existing `grid` / `force` shells still adds its visible panel
entries by editing the typed `PREVIEW_PANEL_REGISTRY` in
`preview-shell/preview-ui-context.ts`.

- This is per-shell-family, not per-engine.
- It is a typed registration surface, not a legacy-JS sink edit.
- It therefore satisfies the literal spec-046 closeout bar of "typed
  registration points rather than `editor.js` / `layout-bridge.js`".

If the repo later wants the stricter "even a new shell family is pure
registration" bar, that should be a new follow-up spec rather than a reason to
reopen spec 046.
...
The exact line count is not the sole goal, but the stale reopen-era
1,601-line `editor.js` / 395-line `layout-bridge.js` state was correctly
treated as unfinished. The reconciled closeout state is a guarded
316-line `editor.js` and 77-line `layout-bridge.js`, with
`scripts/check-preview-shell-size-budgets.mjs` enforcing the thin-adapter
budgets (`editor.js` <= 320 non-empty lines, `layout-bridge.js` <= 80).
...
8. **False-closeout veto**
   This veto was the correct reopen criterion. The package is only considered
   closed because the reconciled answer is now "no, we would not have to widen
   the legacy browser shell" to add 50/150/500 engines.
