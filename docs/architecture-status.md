...
Figma-like nested frame system with direction, gap, padding, per-axis sizing (HUG/FILL/FIXED), and 9-point alignment. Two-pass engine: measure (bottom-up) → place (top-down). **240** vitest tests in `packages/layout-engine/` (12 known `test-deep-nesting` width parity failures); Python tests cover YAML and legacy parity. **32** v3 Frame YAML definitions in `scripts/diagrams/frames/`. Interactive layout is TS-only; preview and batch SVG are TS export only (spec 012 complete).
