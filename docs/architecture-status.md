...
- `apps/preview/src/server.ts` – Node preview app and persistence/API front door
...
Figma-like nested frame system with direction, gap, padding, per-axis sizing (HUG/FILL/FIXED), and 9-point alignment. Two-pass engine: measure (bottom-up) → place (top-down). Validation lives primarily in `packages/layout-engine/` Vitest coverage plus focused preview-app tests. **11** canonical v3 Frame YAML definitions in `diagrams/1.input/` (spec 020 invariant pack). Interactive layout is TS-only; preview and batch SVG are TS export only (spec 012 complete).
