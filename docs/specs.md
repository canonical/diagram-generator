...
| 030 Sequence layout | `specs/030-sequence-layout/` | Draft | First Mermaid-heavy direct port: a first-party TypeScript `sequenceDiagram`-style lane through the preview-engine architecture, with Mermaid kept as an adapter surface rather than the runtime renderer. |
