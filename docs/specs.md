...
| 048 ELK sizing and interaction follow-up | `specs/048-elk-sizing-interaction-followup/` | **REOPENED (2026-06-28)** — live ELK-resize "relayout failed" P0 unproven on real UI; fix owned by spec 065 T022 | Current ELK product follow-up: preserve Fill/Hug/Fixed semantics, rerun text measurement on width changes, add live resize feedback, align parent text insets, and harden ELK option/debug surfaces without reopening 046. |
| 051 Preview editor contextual aside | `specs/051-preview-editor-contextual-aside/` | **Active (2026-06-28)** | Figma-like right-aside cleanup. Phase 1–7 unit facts pass, but author still sees N/A controls disabled-not-hidden, ELK options not algorithm-contextual, and the debug overlay present. New Phase 8 + spec 065 protocol §2 gate. |
| 065 Interactive relayout contract | `specs/065-interactive-relayout-contract/` | **Active / blocking** | One typed `PreviewRenderIntent` that render/relayout/direction/panel-sync all read. Owns the real fix behind the post-load fidelity loop; unblocks 060/057/048/051. Closing gate: `verification-protocol.md`. |
...
| 057 Graph engine fidelity and example fit | `specs/057-graph-engine-fidelity-and-example-fit/` | **REOPENED (2026-06-28)** — real-layout probes stand; mongo/tiered browser re-verification must be a real UI gesture per spec 065 protocol §2 | Engine/example fit now has real preview-engine fidelity probes, stricter unsupported-engine exposure gates, and browser evidence for the reported Mongo/Tiered fixtures. |
...
| 060 Output pane engine tabs and live rerender | `specs/060-output-pane-engine-tabs-rerender/` | **REOPENED (2026-06-28)** — engine identity real; direction-flip (T022) + Playwright (T040) closed on a `skipModelUpdate` fake proof; re-earn via spec 065 protocol §2 | Engine switch now commits active engine intent into the frame tree before render, stamps `data-layout-engine`, and has real-render plus Playwright identity evidence. See `docs/spec-reviews/branch-060.md`. |
