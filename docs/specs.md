...
| 080 Renderable interchange import | `specs/080-renderable-interchange-import/` | Draft | Capability-based interchange import: structural loss (dropped edges/nodes/containment/direction/multiplicity) blocks before write through one shared gate; inline-declared edges and subgraph-local direction import faithfully; reverse direction and engine selection are model/lowering tasks; tokenizer/parser/IR replaces per-line regex; phased D2 parity. Authored from the 2026-07-18 Opus review. |
