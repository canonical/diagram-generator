...
| 061 Preview grid regression investigation | `specs/061-preview-grid-regression/` | Review | Shared typed grid capability gate now drives the grid panel, 9-dot inspector, and inert runtime boundary. `findings.md` records that the typed V3 overlay path remains intact and retires grid affordances on non-grid engines; preview validation has one pre-existing ELK option-default failure. |
