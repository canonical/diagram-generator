...
| 050 Preview editor recovery | `specs/050-preview-editor-recovery/` | Closeout Ready | Post-046 editor regressions recovered across bootstrap, stage interaction (drag/resize/nudge/delete/undo), inspector controls (single + multi-select), engine controls, text edit, and save/reload/export, all live-verified on `preview-smoke` and locked with focused tests. No legacy preview JS widened (`editor.js`/`layout-bridge.js` byte-identical to `main`). |
