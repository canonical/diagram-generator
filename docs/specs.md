...
| 062 Parent/child hug resize propagation | `specs/062-parent-child-hug-resize-propagation/` | Active — review hardening landed 2026-07-03 | Nested `HUG` container children now recompute width under a smaller parent and repo-owned regressions cover that generic path, but the package stays active until the full SC-005 validation battery is rerun in a fully installed worktree. |
