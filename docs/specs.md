...
| 057 Graph engine fidelity and example fit | `specs/057-graph-engine-fidelity-and-example-fit/` | Closeout Ready | Engine/example fit now has real preview-engine fidelity probes, stricter unsupported-engine exposure gates, and browser evidence for the reported Mongo/Tiered fixtures. |
...
| 059 Cross-document style source of truth | `specs/059-cross-document-style-source-of-truth/` | Draft (not started) | Unify shared box rhythm and engine identity across document kinds. **No commits exist on `feat/059`**; status corrected from In Progress per `docs/spec-reviews/branch-059.md`. |
| 060 Output pane engine tabs and live rerender | `specs/060-output-pane-engine-tabs-rerender/` | Closeout Ready | Engine switch now commits active engine intent into the frame tree before render, stamps `data-layout-engine`, and has real-render plus Playwright identity evidence. See `docs/spec-reviews/branch-060.md`. |
