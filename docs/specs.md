...
| 072 Preview engine hardening | `specs/072-preview-engine-hardening/` | Active | Shared preview-engine hardening slice: remove stale engine chrome, restore engine-switch canvas padding parity, generalize sequence/document-kind handling out of central owners, automate builtin install-unit registration, and add a save→reload→switch-back browser proof for per-engine state. |
