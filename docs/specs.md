...
| 046 Editor host endgame | `specs/046-editor-host-endgame/` | In Progress | Active preview-shell closeout package: future engine onboarding no longer needs to widen `editor.js` or `layout-bridge.js`, but the residual `editor.js` host is still too large to satisfy the spec's literal thin-bootstrap and 150-engine proof bars. |
