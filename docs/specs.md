...
| 047 Render IR unification | `specs/047-render-ir-unification/` | In Progress | Active render-convergence package: shared frame-arrowhead geometry now lives in a neutral helper, parity coverage compares arrowhead geometry and short-segment shaft truncation, and the next slices are fresh-render/bridge cutover onto one geometry authority with separate export and preview serializers. |
