...
| 066 Graph engine layout option surfacing | `specs/066-graph-engine-layout-option-surfacing/` | Closeout Ready | Official Dagre + non-layered ELK option inventory for the preview aside, plus the folded-in parameter-pane architecture closeout: operator-scoped session buckets, one effective-override resolver, one canonical graph-layout pane host, active-engine reload seeding, and manifest-aware shared-namespace validation. |
| 067 Layout engine parameter pane | `specs/067-layout-engine-parameter-pane/` | Folded into 066 | Review artifact retained for architecture notes and adversarial findings. Do not implement on a separate branch unless 066 closes and a new scope genuinely remains. |
