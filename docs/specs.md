...
| 076 Port Mermaid cluster/ELK lowering (TLS example) | `docs/spec-archive/076-tls-mermaid-cold-start-fit/` | Merged to `main` and archived 2026-07-07 | The bounded ELK-only follow-up landed as a generic typed lowering shim: authored/native compounds keep local direction, invisible ordering edges preserve the cert/endpoint rows without Dagre, and the TLS fixture now resolves to `elk-layered` with repo-owned compatibility + geometry regressions. |
