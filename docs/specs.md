...
| 076 Port Mermaid cluster/ELK lowering (TLS example) | `specs/076-tls-mermaid-cold-start-fit/` | Draft | Reach Mermaid's grouping/parenting by porting the MIT `@mermaid-js/layout-elk` cluster->ELK compound lowering (same `elkjs` this repo ships) instead of reintroducing Dagre. Blocking T0 spike proves it on the TLS fixture before the port; Opus-reviewed (`docs/spec-reviews/076-...md`). spec.md + plan.md + tasks.md + review + asset pack. |
