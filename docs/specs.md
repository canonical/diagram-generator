...
| 062 Parent/child hug resize propagation | `specs/062-parent-child-hug-resize-propagation/` | Active — adversarial review reopened 2026-07-03 | The landed fix covers the leaf `small_box` repro on `test-alignment-grid`, but a nested `HUG` container child still retains stale width under a smaller parent; restore closeout only after the generic child-container contract is fixed and covered. |
