...
| 076 Port Mermaid cluster/ELK lowering (TLS example) | `specs/076-tls-mermaid-cold-start-fit/` | Closeout Ready | The bounded ELK-only follow-up landed as a generic typed lowering shim: authored/native compounds keep local direction, invisible ordering edges preserve the cert/endpoint rows without Dagre, and the TLS fixture now resolves to `elk-layered` with repo-owned compatibility + geometry regressions. |
