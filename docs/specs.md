...
| 069 Editor mutation state determinism | `specs/069-editor-mutation-state-determinism/` | Closeout Ready | Defines and verifies the transaction/state-vector contract so editor gestures cannot leave active tab, render intent, option buckets, dirty/undo state, rendered SVG, save payload, and reloaded YAML disagreeing. |
