...
| 076 TLS certificate provider Mermaid cold-start fit | `specs/076-tls-mermaid-cold-start-fit/` | Draft | Example-owned cold-start pack for the Mermaid-origin TLS topology: preserve the source/comparison assets, seed a sibling Mermaid recreation, and make future ELK claims clearable only through fixture-owned compatibility + geometry evidence rather than by reopening Dagre. |
