...
| 075 Preview folder workspaces | `specs/075-preview-folder-workspaces/` | In progress — browser safety/UX slice implemented | Typed multi-root sources plus browser Open folder now provide grouped qualified nav, multiple persisted handles, reconnect UX, authoritative handle-gated saves, local conflict protection, read-only enforcement, and bounded ingest. Real picker evidence, Save a copy, server-root conflict handling, first-run polish, and temp-cache disposal remain before closeout. |
