...
| 076 Port Mermaid cluster/ELK lowering (TLS example) | `specs/076-tls-mermaid-cold-start-fit/` | Active / blocked on bounded ELK ordering follow-up | Reach Mermaid's grouping/parenting by porting the MIT `@mermaid-js/layout-elk` cluster->ELK compound lowering (same `elkjs` this repo ships) instead of reintroducing Dagre. The 2026-07-06 T0 spike and Mermaid oracle both preserved compounds but failed the reference ordering; Opus's 2026-07-07 review rejected the Dagre pivot and requires a bounded ELK ordering / crash-root-cause follow-up before Phase 2. |
