...
| 068 Internal dual-path deletion | `specs/068-internal-dual-path-deletion/` | Closeout Ready | Repo-owned preview-shell, persistence, route, and browser-entry dual paths have been deleted in active source/tests; search evidence and full validation now pass, with fixture/engine-option isolation documented for the mutable authored fixture smell. |
| 069 Editor mutation state determinism | `specs/069-editor-mutation-state-determinism/` | Draft | Define a transaction/state-vector contract so editor gestures cannot leave active tab, render intent, option buckets, dirty/undo state, rendered SVG, save payload, and reloaded YAML disagreeing. Implement on `feat/069-editor-mutation-state-determinism`, not on the 068 branch. |
