...
| 046 Editor host endgame | `specs/046-editor-host-endgame/` | In Progress | Reopened preview-shell closeout package: do not close until the honest answer is yes for scaling to 50, 150, and 500 heterogeneous layout engines without widening legacy browser-shell integration sinks, and do not treat `scripts/preview/*.js` as a valid growth surface for new behavior-heavy preview logic. |
