...
| 077 Mermaid ELK cluster lowering port (layout-only) | `specs/077-mermaid-elk-cluster-lowering-port/` | Active / blocked on T054 | Implementation is merged to `main`; T051 portability is complete, while T054 remains open pending clean full layout-engine and preview baselines. Keep the package open; do not claim closeout or archive it yet. |
