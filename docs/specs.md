...
| 052 Layout engine onboarding factory and multi-engine port | `specs/052-layout-engine-onboarding-port/` | Closeout Ready | Declarative `defineGraphLayoutPreviewEngine` factory + per-engine contract-test generator + bundle-freshness/no-central-branching gates; ports product-suitable elkjs algorithms and adds dagre (mermaid's layout core). Full validation and no-screenshot DOM matrix are recorded in `tasks.md`; future same-package engines should land one per commit to demonstrate the literal SC-007 diff stat. |
