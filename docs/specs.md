...
| 077 Mermaid ELK cluster lowering port (layout-only) | `specs/077-mermaid-elk-cluster-lowering-port/` | Draft — proposed | Port Mermaid's layout-only cluster→ELK lowering into TS on the existing `elkjs` (keep our renderer; do not add `@mermaid-js/layout-elk`). Compound graph + per-compound direction + LCA cross-cluster edge attachment + native model order; read ELK geometry verbatim. Hard guardrails ban post-ELK box-moving and bespoke arrow routing; render-level gate replaces snippet asserts. spec.md + plan.md + tasks.md drafted. |
