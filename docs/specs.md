...
| 044 Preview shell architecture follow-up | `specs/044-preview-shell-architecture-followup/` | Closeout Ready | Post-043 contract package: browser namespaces, migration rules, bundle boundaries, and `layout-bridge.js` decomposition are landed; `browser-entry.ts` now exposes only `core`, `previewBridge`, `previewEngines`, and `previewShell`, and the remaining legacy wrappers consume only those owner-scoped seams. |
| 045 Preview host engine modularity | `specs/045-preview-host-engine-modularity/` | Closeout Ready | Preview-app modularity package: typed lane descriptors, shared viewer/index page shell, registered host modules/routes, builtin host runtime plus module-owned install contexts, and a production-path third-lane coexistence proof. Route/page growth now stays in `preview-host/`; deps-heavy builtin lanes still extend the builtin host runtime until later context self-registration exists. |
...
| 047 Render IR unification | `specs/047-render-ir-unification/` | Draft | Next preview-architecture package: collapse duplicated preview/export geometry emitters toward one IR authority with separate export-SVG and preview-DOM serializers. |
