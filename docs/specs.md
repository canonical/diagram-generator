...
| 028 Diagram interchange (Mermaid & D2) | `specs/028-diagram-interchange-mermaid-d2/` | Draft | Bidirectional interchange: Mermaid/D2 import to canonical YAML, fidelity matrix, export hardening, and round-trip CLIs; builds on the archived spec 022 adapters. |
...
| 061 Preview grid regression investigation | `specs/061-preview-grid-regression/` | Draft | Hide grid affordances on non-grid engines now (stops the ELK relayout error), then root-cause and record a restore-or-retire decision for the lost layout grid. spec.md + plan.md + tasks.md drafted; investigation writes `findings.md`. |
...
| 075 Preview folder workspaces | `specs/075-preview-folder-workspaces/` | Draft | Replace the single `DG_FRAMES_DIR` model with a typed `DiagramWorkspaceSource` abstraction plus two adapters: multi-root server folders and a browser "Open folder" (File System Access) source for local, cloud-synced, no-account diagram folders. Grouped/qualified nav, read-only bundled examples, per-source persist->reload. spec.md + plan.md + tasks.md drafted. |
| 076 Port Mermaid cluster/ELK lowering (TLS example) | `specs/076-tls-mermaid-cold-start-fit/` | REOPENED 2026-07-07 — closeout premature | The prior closeout passed weak tests (engine resolution + two geometry snippets) but the rendered diagram fails the visual bar: grey two-line annotation nodes collapse to single-line bare text, the top-down clustered structure is cramped, and text truncates. Active work is Phase 5 (`T050`–`T056`) in the spec package; the gate now requires a render-level regression + documented parity vs the Mermaid reference. Do not reintroduce Dagre. |
| 077 YAML frame → draw.io export | `specs/077-yaml-drawio-export/` | Active | TypeScript batch exporter: frame YAML → layout → mxGraph `.drawio` under `diagrams/2.output/draw.io/`; ports draw.io style presets; ai-infra fixture regressions. |
| 078 Figma autolayout plugin | `docs/spec-archive/078-figma-autolayout-plugin/` | Merged to `main` and archived 2026-07-10 | Local-development Figma Design plugin that recreates diagram-generator nodes as native editable auto-layout, including effective Fill/Hug/Fixed sizing from the preview/YAML layout engine and a Figma readback validator. |
| 079 Figma component variant import | `specs/079-figma-component-variant-import/` | Draft | Follow-up Figma importer: map semantic boxes to user-authored native component variants, populate direction-aware component slots, and replace demo buttons with arbitrary YAML file import. |
