...
| 080 Renderable interchange import | `specs/080-renderable-interchange-import/` | Review — implementation complete | Capability-based Mermaid and D2 import now blocks structural loss through one shared gate; tokenizer/parser/IR handles compound topology, reverse/local direction, supported styles, and bounded input; capability-driven v3/ELK selection persists across reload; server-root and local-folder write paths have regression coverage. Awaiting Opus adversarial re-review. |
