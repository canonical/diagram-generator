...
| 077 YAML frame → draw.io export | `specs/077-yaml-drawio-export/` | Closeout pending — dark-system-theme handling under research | TypeScript batch exporter: frame YAML → layout → mxGraph `.drawio` under `diagrams/2.output/draw.io/`; ports draw.io style presets; ai-infra fixture regressions; must define a pinned-light or intentional adaptive draw.io theme contract before manual closeout. |
