...
| 036 Headingless wrapper contract | `specs/036-headingless-wrapper-contract/` | Draft | Restore the rule that a container without a heading behaves like a structural wrapper rather than a padded/styled panel, and prefer that contract fix over adding a new pass-through variant. |
| 037 Preview engine drift closeout | `specs/037-preview-engine-drift-closeout/` | Draft | Close the engine-surface drift from the 2026-06-06 architectural review: align accepted engine ids with hostable runtime lanes, converge force save on canonical persisted state, remove forbidden localStorage usage, and add typed compatibility groundwork for spec 035. |
