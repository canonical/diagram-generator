...
| 055 Preview engine workspace navigation | `specs/055-preview-engine-workspace-navigation/` | In Progress | Engine switching needs a typed workspace model: compatible-engine tabs/prev-next navigation, explicit reopen/save semantics across engine tabs, and hidden inactive engine chrome instead of noisy disabled leftovers. |
| 056 Arrow reroute structural mutations | `specs/056-arrow-reroute-structural-mutations/` | In Progress | Structural edits such as page-direction changes and resize interactions must invalidate and recompute routed arrows through a typed reroute trigger contract so attachments survive live edit and reload. |
