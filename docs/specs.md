...
| 076 Port Mermaid cluster/ELK lowering (TLS example) | `specs/076-tls-mermaid-cold-start-fit/` | Closeout Ready / review-reopen resolved 2026-07-07 | The review-driven Phase 6 follow-up is landed: TLS cert rows stay in the ELK graph, `tls_provider` now wraps the lower clusters in both YAML and reference `.mmd`, preview-wire `justify` survives browser round-trip, and the new server/browser TLS regressions are green. Do not reintroduce Dagre. |
