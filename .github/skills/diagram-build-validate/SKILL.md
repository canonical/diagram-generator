...
- **Gutter consistency.** Only two gap scales exist: `compact-gap` (8px) for tightly packed boxes inside a panel, and `grid-gutter` (24px) for all structural column/row gaps. Do not use 16px or 32px gaps — they create inconsistent gutter lanes across nesting levels. The same 24px gutter must be used at every nesting level where columns or rows are structural.
