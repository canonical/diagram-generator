...
- **Sentence case.** All diagram text uses sentence case: capitalize the first word and proper nouns only. Do not use Title Case for headings, labels, or annotations. Exceptions: product names, acronyms, and text that the source explicitly styles differently. Convert Title Case from rough sketches to sentence case during redraw.