...
test("editor bootstrap tail accepts the namespaced previewShell.bootstrap contract", () => {
  ...
  const context = {
    ...
    LayoutEngine: {
      previewShell: {
        bootstrap: {
          bootstrapPreviewEditorHost(options: Record<string, unknown>) {
            capturedOptions = normalizeVmValue({
              ...
              currentPath: (options.getCurrentPath as () => string)(),
              hasAttemptNavigation: typeof options.attemptNavigation,
              hasInitNavTabs: typeof options.initNavTabs,
              gridOverrides: (options.getGridOverrides as () => unknown)(),
              elkLayoutOverrides: (options.getElkLayoutOverrides as () => unknown)(),
              removedIds: Array.from((options.getRemovedIds as () => Set<string>)()),
              frameTree: (options.getFrameTree as () => unknown)(),
              rootId: (options.getRootId as () => string)(),
              selectedIds: (options.getSelectedIds as () => string[])(),
              ...
              constraintErrors: (options.getConstraintErrorCount as () => number)(),
              allowInternalDirtyNavigation: (options.allowInternalDirtyNavigation as () => boolean)(),
              overrideToolbarSlug: (options.overrideToolbar as Record<string, unknown>).slug,
              overrideToolbarMessage: (options.overrideToolbar as Record<string, unknown>).confirmClearAllMessage,
              generation: (options.getGeneration as () => number)(),
              hasEventSourceFactory: typeof options.eventSourceFactory,
              hasSetBuildStatus: typeof options.setBuildStatus,
            });
          },
        },
      },
  ...
  assert.deepEqual(capturedOptions, {
    ...
    overrideToolbarSlug: "demo",
    overrideToolbarMessage: "Clear all overrides for demo?",
    generation: 3,
    hasEventSourceFactory: "function",
    hasSetBuildStatus: "function",
  });
});
