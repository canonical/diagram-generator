...
- **Current handoff note:** spec 053 is merged to `main` and archived under `docs/spec-archive/053-preview-editor-post-refactor-correctness/`. Spec 054 is now **Closeout Ready** and merged to `main`: `packages/layout-engine/src/preview-shell/preview-override-model.ts` owns canonical save-payload assembly for frame/arrow/grid/removal/engine-layout state, emitted keys are filtered through the shared `frame-override-manifest.ts`, `preview-arrow-component-ids.ts`, and `frame-yaml-engine-layout-contract.ts` owners, `app-save-payload.ts` is reduced to a guard/assertion layer, and repo-owned persist->reload coverage now explicitly covers canonical frame moves/resizes plus duplicate/explicit/branch arrow waypoint saves and removals. A 2026-06-27 follow-up intentionally committed authored engine choices for `example-platform-architecture`, `mongo-octavia-ha`, `preview-smoke`, and `support-engineering-flow`; the preview-host and engine-registry contracts now follow YAML-on-disk truth, and full validation is green on `main`. Specs 055-059 have been promoted from draft to **In Progress** for an overnight SpecKit run; local feature branches must remain one-spec-per-branch, and the queue is expected to execute in numeric order via `feat/055...` through `feat/059...`. Spec 056 is now **Closeout Ready** on `feat/056-arrow-reroute-structural-mutations`: the dedicated reroute flow map lives at `specs/056-arrow-reroute-structural-mutations/preview-reroute-flow.md`, route-bearing frame overrides invalidate stale authored arrow geometry during relayout/fresh render, save payloads clear stale authored/manual waypoint geometry on persist, and reload preserves explicit arrow ids plus `arrow:<id>` attachment targets with full validation green.
