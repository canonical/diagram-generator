...
- **Active spec (when relevant):** `specs/050-preview-editor-recovery/` is the highest-priority active repair package on `feat/050-preview-editor-recovery`: recover post-046 preview-editor UI regressions across bootstrap, stage interaction, inspector controls, engine controls, save/reload, and export without widening legacy preview JS. `docs/spec-archive/044-preview-shell-architecture-followup/` and `docs/spec-archive/045-preview-host-engine-modularity/` are merged and archived on `main`. `specs/047-render-ir-unification/` remains **Closeout Ready** on `feat/047-render-ir-unification`: fresh preview and export SVG both consume the shared display-list IR path, `svg-render.ts` is a thin compatibility wrapper over the shared serializers, and the remaining bridge/waypoint lanes are explicitly preview-only mutation owners over shared arrow geometry primitives rather than separate renderers. `specs/046-editor-host-endgame/` remains architecturally closed; do not reopen it unless future work widens `scripts/preview/editor.js`, `scripts/preview/layout-bridge.js`, or reintroduces central preview-host/document-kind/engine branching. Use `docs/specs.md` as the active spec index; `TODO.md` is only a pointer.
...
- **Current handoff note:** spec 050 progress this session (branch `feat/050-preview-editor-recovery`): fixed the editor highlight style to render as a bordered black leaf so it keeps the 64px box height (matched DIAGRAM.md; resolved the `PREVIEW_STYLE_SEMANTICS.highlight` vs `VARIANT_OVERLAYS.highlight` divergence). Live-verified T010 (browser-entry contracts + console-error-free load), T011 (picker next-step nav), T012 (inspector relayout keeps selection chrome), and T013 delete/undo/redo with faithful Playwright input. Removed stray root `image.png` (T057). Open in 050: T013 drag-reorder/resize/live-resize probes (drag registered an override but did not visibly reorder an autolayout child — needs deeper probe), T014, T020/T021 inspector controls, T023, T030–T033 engine controls, T040–T043 save/reload/export. Disposable live-probe slug is `preview-smoke`. Engine suite 754 green, preview suite 114 green. Note: the AGENTS.md handoff history also mentions possible unrelated uncommitted YAML edits in `scripts/diagrams/frames/example-*.yaml` — keep them out of 050 commits.
