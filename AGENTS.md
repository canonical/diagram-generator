...
- **Current handoff note:** specs 053-060 have product closeout complete. Spec 066 is merged to `main` and archived; spec 067 remains archived as a folded review artifact. Specs 068 and 069 are merged to `main`, and spec 069 is archived under `docs/spec-archive/069-editor-mutation-state-determinism/`: all tasks are complete, adversarial review gaps were addressed, browser evidence is `ok: true`, SC-001 probe coverage is hash-guarded in spec validation, and full validation passed. Spec 071 remains **Closeout Ready** on `feat/071-preview-render-node-graph` with the no-central-branching onboarding proof, post-migration render-path inventory, and full green validation summary committed. Spec 062 is back to **Closeout Ready** on `feat/062-parent-child-hug-resize-propagation` after the review-hardening rerun revalidated the nested non-leaf `HUG` fix and the unconstrained root/top-level `HUG` follow-on fix. Spec 063 remains **Closeout Ready** on `feat/063-auto-style-by-nesting-depth`. Spec 072 is merged to `main` and archived under `docs/spec-archive/072-preview-engine-hardening/`: the render-node fit seam is structurally guarded, the stale active-engine badge path is removed, section-heading bottom spacing is increased through the shared style contract, browser persistence/padding proofs are live, and Playwright-backed preview regressions now skip cleanly when chromium is unavailable. Spec 073 is now **Closeout Ready** on `feat/073-layout-node-model-param-unification`: `grid` normalized to canonical `frame` with a boundary alias, lane panels register through data contributions instead of the old central `PREVIEW_PANEL_REGISTRY`, force parameters now flow through the shared layout-params controller while `force-spec` routes/persistence stay intact, the no-`algorithmFamily` guard is locked in tests, and the remaining force host/route/persistence convergence is intentionally deferred to a dedicated follow-up spec instead of being bundled into phase 1. Validation is green for spec 073 via `npm --prefix packages/layout-engine test`, `npm --prefix apps/preview test`, `node scripts/check_no_new_python.mjs`, and `node scripts/check-preview-shell-size-budgets.mjs`; `build:browser` also passed after the export changes. `INBOX.md` still carries the recurring raw notes by explicit user instruction, so closeout/admin cleanup there remains separate from the merged product work. The remaining Opus-ordered queue in `TODO.md` is now 061 -> 064.
