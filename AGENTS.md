...
- **Current handoff note:** spec 053 is merged to `main` and archived under `docs/spec-archive/053-preview-editor-post-refactor-correctness/`. Its delivered surface includes the preview editor correctness fixes, preview save hardening, duplicate-arrow persistence disambiguation, and the preview-routing identity split that preserves authored `arrow.id` for `arrow:<id>` / `@id` attachment routing while carrying preview component ids separately for selection/save. Validation is green on the full documented commands: `npm --prefix packages/layout-engine test`, `npm --prefix apps/preview test`, `npm --prefix packages/layout-engine run build:browser`, `node scripts/check-browser-bundle-fresh.mjs`, and `node scripts/check_no_new_python.mjs`. The next wider follow-up is the typed persistence-model migration out of `scripts/preview/component-model.js`; open that only on a matching `feat/054-preview-persistence-model-typescript` branch with a real spec package, not as stray work on `main`.
