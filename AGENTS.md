...
- **Active spec (when relevant):** `specs/044-preview-shell-architecture-followup/` is now **Closeout Ready** on `feat/044-preview-shell-architecture-followup`. `packages/layout-engine/src/browser-entry.ts` now exposes only `core`, `previewBridge`, `previewEngines`, and `previewShell`; the remaining legacy browser wrappers in `scripts/preview/` resolve only those namespaced owners; and contract coverage is split across owner-scoped suites in `packages/layout-engine/tests/browser-entry-contract.test.ts` and `apps/preview/src/persistence/*.test.ts`. `specs/045-preview-host-engine-modularity/` remains **Closeout Ready** on its matching branch. `specs/046-editor-host-endgame/` remains architecturally closed; do not reopen it unless future work widens `scripts/preview/editor.js`, `scripts/preview/layout-bridge.js`, or reintroduces central preview-host/document-kind/engine branching. `specs/047-render-ir-unification/` stays parked until a later render-convergence slice is explicitly justified. Use `docs/specs.md` as the active spec index; `TODO.md` is only a pointer.
