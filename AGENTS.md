...
- **Active spec (when relevant):** `specs/048-elk-sizing-interaction-followup/` is now **Closeout Ready** on `feat/048-elk-sizing-interaction-followup`. The ELK feature work, targeted owning tests, `packages/graph-layout-elk`, and `apps/preview` validations are complete; the only remaining package-wide red is the pre-existing unrelated `packages/layout-engine/tests/parity.test.ts` native headed/body fixture bucket. `specs/046-editor-host-endgame/` has its closeout bar met; the honest answer is now **yes** for adding **50, 150, and 500 heterogeneous layout engines** without widening `editor.js`, `layout-bridge.js`, or central preview-host/document-kind sinks. Adjacent follow-on work in `specs/044-preview-shell-architecture-followup/`, `specs/045-preview-host-engine-modularity/`, `specs/047-render-ir-unification/`, and future ELK follow-ups should stay independent and should not reopen 046 unless they reintroduce behavior-heavy legacy JS ownership or ELK-only substrate contracts. Use `docs/specs.md` as the active spec index; `TODO.md` is only a pointer.
