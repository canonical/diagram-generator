...
- **Active spec (when relevant):** Use `docs/specs.md` as the active spec index; `TODO.md` is only a pointer. `specs/055-preview-engine-workspace-navigation/` is now **Closeout Ready** on `feat/055-preview-engine-workspace-navigation`. `specs/054-preview-persistence-model-typescript/` is **Closeout Ready** and merged to `main`; archive it after branch cleanup. `specs/056-arrow-reroute-structural-mutations/`, `specs/057-graph-engine-fidelity-and-example-fit/`, `specs/058-layer-tree-inspector-selection-ergonomics/`, and `specs/059-cross-document-style-source-of-truth/` are **In Progress** and should run only from their matching `feat/056...` through `feat/059...` branches. `specs/052-layout-engine-onboarding-port/` is **Closeout Ready** on `feat/052-layout-engine-onboarding-port`. `specs/051-preview-editor-contextual-aside/` remains **Draft** for right-aside UI cleanup. `specs/047-render-ir-unification/` remains **Closeout Ready** on `feat/047-render-ir-unification`. `specs/048-elk-sizing-interaction-followup/` is **Closeout Ready**. `specs/046-editor-host-endgame/` remains architecturally closed; do not reopen it unless future work widens `scripts/preview/editor.js`, `scripts/preview/layout-bridge.js`, or reintroduces central preview-host/document-kind/engine branching. `specs/053-preview-editor-post-refactor-correctness/` was merged to `main` and archived under `docs/spec-archive/053-preview-editor-post-refactor-correctness/`. `specs/050-preview-editor-recovery/` was merged to `main` and archived under `docs/spec-archive/050-preview-editor-recovery/`.
- **Latest 046 batch:** the final substrate and closeout pass is complete on this branch. `editor.js` is now a 256-line browser adapter over the typed grid install/runtime seam, `layout-bridge.js` is now an 88-line compat bridge over the typed preview-bridge install runtime, `packages/layout-engine/src/preview-shell/index.ts` is a seven-line top-level fan-out into owner-scoped barrels, `packages/layout-engine/src/browser-entry.ts` is an eight-line top-level fan-out into split browser-entry barrels, and the old monolithic VM harness is replaced by owner-scoped contract suites backed by `preview-script-test-helpers.ts`. The foreign-shaped install proof is now real through `mindmap-lite-install-unit.test.ts`, and `graph-layout-core` is now engine-open enough for Mermaid/D2/Dagre-class onboarding via generic engine ids, capability descriptors, object insets, side/point-anchored ports, and explicit size semantics adapted at the ELK boundary.
...
- **Current handoff note:** spec 053 is merged to `main` and archived under `docs/spec-archive/053-preview-editor-post-refactor-correctness/`. Spec 054 is now **Closeout Ready** and merged to `main`: `packages/layout-engine/src/preview-shell/preview-override-model.ts` owns canonical save-payload assembly for frame/arrow/grid/removal/engine-layout state, emitted keys are filtered through the shared `frame-override-manifest.ts`, `preview-arrow-component-ids.ts`, and `frame-yaml-engine-layout-contract.ts` owners, `app-save-payload.ts` is reduced to a guard/assertion layer, and repo-owned persist->reload coverage now explicitly covers canonical frame moves/resizes plus duplicate/explicit/branch arrow waypoint saves and removals. A 2026-06-27 follow-up intentionally committed authored engine choices for `example-platform-architecture`, `mongo-octavia-ha`, `preview-smoke`, and `support-engineering-flow`; the preview-host and engine-registry contracts now follow YAML-on-disk truth, and full validation is green on `main`. Spec 055 is now **Closeout Ready** on `feat/055-preview-engine-workspace-navigation`: `packages/layout-engine/src/preview-shell/preview-engine-workspace.ts` owns the typed active/persisted/compatible engine workspace model plus reopen/save semantics, `preview-engine-workspace-chrome.ts` now keeps engine-tab switches browser-local until Save while feeding the active engine back into the save client + panel-visibility runtime, `scripts/preview/engine-switcher.js` remains a thin typed bootstrap wrapper, and preview-host wiring now shows active engine identity for sequence documents while passing `document_kind` / active-engine config through the runtime shell. Validation bootstrap now prebuilds graph-layout deps plus the layout-engine node/browser artifacts before the advertised test commands run. Specs 056-059 remain **In Progress** and should continue only from their matching feature branches.
