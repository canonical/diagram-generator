...
- **Active spec (when relevant):** `specs/050-preview-editor-recovery/` is **Closeout Ready** on `feat/050-preview-editor-recovery`: post-046 preview-editor regressions are recovered across bootstrap, stage interaction (drag-reorder/resize/nudge/delete/undo/redo), inspector controls (single + multi-select), engine controls, text edit, and save/reload/export. Every recovery-matrix surface is now Pass or explicitly triaged (arrow/waypoint editing is out-of-scope, contract-covered). All fixes landed in typed preview-shell owners — `scripts/preview/editor.js` and `scripts/preview/layout-bridge.js` are byte-identical to `main` (no legacy JS widened). Full suites green: layout-engine 760, apps/preview 114, no-new-python ratchet ok. `docs/spec-archive/044-preview-shell-architecture-followup/` and `docs/spec-archive/045-preview-host-engine-modularity/` are merged and archived on `main`. `specs/047-render-ir-unification/` remains **Closeout Ready** on `feat/047-render-ir-unification`. `specs/046-editor-host-endgame/` remains architecturally closed; do not reopen it unless future work widens `scripts/preview/editor.js`, `scripts/preview/layout-bridge.js`, or reintroduces central preview-host/document-kind/engine branching. Use `docs/specs.md` as the active spec index; `TODO.md` is only a pointer.
...
- **Trap files (search, then partial read):** `scripts/preview/force.js` (~1,600 lines), `packages/layout-engine/dist/layout-engine.iife.js`.
- **Current handoff note:** spec 050 driven to closeout-ready this session (branch `feat/050-preview-editor-recovery`). Phases 2–6 complete with live verification on `preview-smoke` plus focused tests at the owning layers. Key fixes: (1) highlight style now a bordered black leaf keeping 64px box height (`f0d8780`); (2) drag-reorder redirects `children_order` to the synthetic body parent (`e6bfa72`); (3) thrown/rejecting relayout adapter now routes to `failRelayout` instead of an unhandled rejection — last good render preserved, error status set (`3d544c7`, +2 SC-003 tests); (4) **text-edit getNode binding bug** — `createPreviewTextEditRuntimeFromHost` forwarded `options.model.get` unbound, so live double-click text editing threw `Cannot read properties of undefined (reading 'get')` on every attempt; fixed via `(cid) => options.model.get(cid)` with a regression test that fails without the fix (`e3d70e4`). Live-verified: single/multi-select inspector, bulk align + bulk delete/undo, engine select (`[v3, elk-layered]`), ELK panel, grid overlay toggle (`W`), text edit commit, save→reload→export round-trip (Min W 320 → YAML → reload canonical → export width 336). `editor.js`/`layout-bridge.js` byte-identical to `main`. Suites: layout-engine **760** green, apps/preview **114** green, no-new-python ok. Fixtures restored (clean tree). **Next:** adversarial review of 050, then merge prep (review, merge, delete branch, archive package under `docs/spec-archive/`). Disposable live-probe slug is `preview-smoke`. Keep any unrelated `scripts/diagrams/frames/example-*.yaml` / `request-to-hardware-stack.yaml` edits out of 050 commits.
