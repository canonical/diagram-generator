...
- **Active spec (when relevant):** `specs/046-editor-host-endgame/` has cleared the structural integration-sink bar on the current branch: future engine onboarding no longer needs to widen `editor.js` or `layout-bridge.js`. It is **not** closed yet, because `editor.js` is still larger than an ideal thin bootstrap even though the browser-shell onboarding proof is now test-backed across representative external, ported-family, and bespoke controller classes. `specs/044-preview-shell-architecture-followup/` remains open for the residual compatibility/barrel cleanup around the preview bridge/browser contract, and `specs/045-preview-host-engine-modularity/` owns Node preview-host route/page modularity. `specs/047-render-ir-unification/` stays gated behind real 046 closeout. `docs/spec-archive/043-preview-shell-editor-ts-extraction/` is extraction-complete and reopens only for regressions. ELK midpoint-port follow-up remains under `specs/042-implicit-elk-side-ports/`. Otherwise see `TODO.md` / `docs/specs.md`.
- **Trap files (search, then partial read):** `scripts/preview/editor.js` (~1,937 lines in the current tree), `scripts/preview/layout-bridge.js` (~544 lines), `packages/layout-engine/dist/layout-engine.iife.js`.
