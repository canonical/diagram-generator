...
- **Active spec (when relevant):** `specs/046-editor-host-endgame/` has cleared the structural integration-sink bar on the current branch: future engine onboarding no longer needs to widen `editor.js` or `layout-bridge.js`. It is **not** closed yet, because `editor.js` is still larger than an ideal thin bootstrap and the 150-engine proof remains architecture-led rather than fully demonstrated. `specs/044-preview-shell-architecture-followup/` remains open for the residual ELK debug/raw-view and compatibility-fallback cleanup in `layout-bridge.js`, and `specs/045-preview-host-engine-modularity/` owns Node preview-host route/page modularity. `specs/047-render-ir-unification/` stays gated behind real 046 closeout. `docs/spec-archive/043-preview-shell-editor-ts-extraction/` is extraction-complete and reopens only for regressions. ELK midpoint-port follow-up remains under `specs/042-implicit-elk-side-ports/`. Otherwise see `TODO.md` / `docs/specs.md`.
- **Trap files (search, then partial read):** `scripts/preview/editor.js` (~2,000 lines in the current tree), `scripts/preview/layout-bridge.js` (~623 lines), `packages/layout-engine/dist/layout-engine.iife.js`.
