...
- **Active spec (when relevant):** `specs/045-preview-host-engine-modularity/` is now **Closeout Ready** on `feat/045-preview-host-engine-modularity`. The preview host now owns typed lane descriptors, shared viewer/index page builders, registered viewer/API/server modules, builtin host runtime/dependency assembly in `apps/preview/src/preview-host/builtin-host-runtime.ts`, module-owned install contexts resolved by key, and a production-path third-lane coexistence proof in `apps/preview/src/persistence/preview-host-contract.test.ts`. `apps/preview/src/server.ts` still owns the generic app entrypoint, HTTP adapters, watch loop, and browser-bundle bootstrap, but it is no longer the default sink for lane/page assembly logic or builtin lane-specific bootstrap contracts. Deps-heavy builtin lanes still extend `builtin-host-runtime.ts` and the single bootstrap call until later module-context self-registration exists. `specs/046-editor-host-endgame/` remains architecturally closed; do not reopen it unless future work widens `scripts/preview/editor.js`, `scripts/preview/layout-bridge.js`, or reintroduces central preview-host/document-kind/engine branching. `specs/044-preview-shell-architecture-followup/` remains a cleanup package, with transitional flat browser-entry alias retirement still open; `specs/047-render-ir-unification/` stays parked until a later render-convergence slice is justified. Use `docs/specs.md` as the active spec index; `TODO.md` is only a pointer.
