...
- **Current handoff note:** spec 053 is merged to `main` and archived under `docs/spec-archive/053-preview-editor-post-refactor-correctness/`. The active follow-up is spec 054 on `feat/054-preview-persistence-model-typescript`: retire JS-owned preview save payload assembly from `scripts/preview/component-model.js`, move override-model ownership into TypeScript, and encode save round-trip coverage as a real closeout gate for persistence work. Do not continue that migration on `main` or on archived 053 paths.
