...
- **Current handoff note:** specs 053-056 are now merged to `main`, and spec 058 is now closeout-ready on `feat/058-layer-tree-inspector-selection-ergonomics`. Spec 058 keeps ownership in typed preview-shell owners: `packages/layout-engine/src/preview-shell/app-shell-panels.ts` now gives layer-tree rows a roving tab stop plus `Enter` / `Shift+Enter` traversal that reselects and refocuses adjacent rows, while `packages/layout-engine/src/preview-shell/frame-style.ts` now infers supported rendered white/grey boxed children as `default` / `parent` before falling back to authored-node heuristics. Focused regressions live in `packages/layout-engine/tests/app-selection-host.test.ts` and `packages/layout-engine/tests/inspector-single-options.test.ts`, and full validation is green on the branch. The next overnight queue should continue with specs 057 and 059 only.
