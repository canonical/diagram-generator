...
- **Active spec (when relevant):** Use `docs/specs.md` as the active spec index; `TODO.md` is only a pointer. `specs/053-preview-editor-post-refactor-correctness/` is **Closeout Ready** on `feat/053-preview-editor-post-refactor-correctness`. `specs/052-layout-engine-onboarding-port/` is **Closeout Ready** on `feat/052-layout-engine-onboarding-port`. `specs/051-preview-editor-contextual-aside/` remains **Draft** for right-aside UI cleanup. `specs/047-render-ir-unification/` remains **Closeout Ready** on `feat/047-render-ir-unification`. `specs/048-elk-sizing-interaction-followup/` is **Closeout Ready**. `specs/046-editor-host-endgame/` remains architecturally closed; do not reopen it unless future work widens `scripts/preview/editor.js`, `scripts/preview/layout-bridge.js`, or reintroduces central preview-host/document-kind/engine branching. `specs/050-preview-editor-recovery/` was merged to `main` and archived under `docs/spec-archive/050-preview-editor-recovery/`.
...
- **Trap files (search, then partial read):** `scripts/preview/component-model.js` (~740 lines; persistence hotspot), `scripts/preview/force.js` (~1,599 lines), `packages/layout-engine/dist/layout-engine.iife.js` (~4.4 MB).
- **Current handoff note:** spec 053 is closeout-ready on `feat/053-preview-editor-post-refactor-correctness`: alignment, direction/stage resize, explicit v3 engine display, compatibility-filtered engine lists, active-manifest engine controls, preview save hardening, and preview-routing identity fixes are implemented. Post-pull validation on 2026-06-26 refreshed two test contracts on this branch: YAML persistence expectations in `apps/preview/src/persistence/frame-diagram.test.ts` now normalize LF/CRLF, and the single-engine v3 preview-host fixture now uses a valid no-arrow frame with page direction so the disabled populated engine field renders. A later same-day follow-up fixed the remaining live inspector regressions: the single-selection variant picker renders again, autolayout leaf alignment targets the effective parent container, and headed-container relayout now mirrors `align` overrides onto the synthetic `__body` node. The persistence hardening pass now canonicalizes transient frame deltas before save, drops synthetic wrapper ids, preserves typed engine-control values, filters unsupported arrow-only save keys without aborting unrelated frame edits, separates reload failures from persist failures, and resolves duplicate arrow ids over the authored arrow sequence. The architectural follow-up fixed the preview-routing identity bug: `routePreviewArrows` keeps authored arrow ids intact for `arrow:<id>` / `@id` attachment routing while carrying preview component ids separately for selection/save. Live DOM verification against `http://127.0.0.1:8101/view/v3:test-alignment-grid` confirmed `small_box` moved from roughly `(439,259)` to `(550,370)` on `BOTTOM_RIGHT`, and variant `parent` changed fill/stroke to `#F3F3F3`. Validation is green on the full documented commands: `npm --prefix packages/layout-engine test`, `npm --prefix apps/preview test`, `npm --prefix packages/layout-engine run build:browser`, `node scripts/check-browser-bundle-fresh.mjs`, and `node scripts/check_no_new_python.mjs`. The in-app browser backend remained unavailable, so browser verification used a headless system Chrome DOM probe.
