...
- **Latest 046 batch:** phase 2c is complete on this branch, phases A/B are green, the install-unit pattern is now documented/tested in `specs/046-editor-host-endgame/install-unit-pattern.md`, the sequence family still has a real typed browser refresh/load proof in `apps/preview/src/persistence/preview-host-contract.test.ts`, the relayout/state-restore/live-resize coordinator cluster now enters through `previewBridge.relayout.createPreviewEditorRelayoutFacadeFromEditorHost(...)`, the stage/pointer/selection-chrome/text-edit/resize/keyboard/runtime-set assembly cluster now enters through `previewShell.bootstrap.createPreviewEditorInteractionFacadeFromEditorHost(...)`, the residual grid/scene/rerender/delete/status-refresh coordinator cluster now enters through `previewShell.scene.createPreviewEditorSceneFacadeFromEditorHost(...)` backed by `app-editor-scene-facade.ts`, the bootstrap/load/navigation/diagram-loaded/bootstrap-tail coordinator cluster now enters through `previewShell.bootstrap.createPreviewEditorBootstrapFacadeFromEditorHost(...)` backed by `app-editor-bootstrap-facade.ts`, `packages/layout-engine/src/preview-shell/index.ts` is now a seven-line top-level fan-out into owner-scoped `preview-shell-*-barrel.ts` files instead of a 1.3k-line barrel, and `packages/layout-engine/src/browser-entry.ts` is now an eight-line top-level fan-out into flat core/preview-shell/preview-engine barrels with the preview-shell flat subset split into owner-scoped browser-entry subset barrels. Spec 046 still remains open because the honest 50/150/500-engine answer is not yet **yes**: `apps/preview/src/persistence/engine-contract-consumers.test.ts` is still the dominant VM harness, and the layout substrate is still not open enough for Mermaid/D2/Dagre-class ports without more work.
- **Trap files (search, then partial read):** `apps/preview/src/persistence/engine-contract-consumers.test.ts` (~3,470 lines), `scripts/preview/force.js` (~1,600 lines), `packages/layout-engine/dist/layout-engine.iife.js`.
