...
- **Current handoff note:** specs 053-060 have product closeout complete. Spec 066 is merged to `main` and archived; spec 067 remains archived as a folded review artifact. Specs 068 and 069 are merged to `main`, and spec 069 is archived under `docs/spec-archive/069-editor-mutation-state-determinism/`: all tasks are complete, adversarial review gaps were addressed, browser evidence is `ok: true`, SC-001 probe coverage is hash-guarded in spec validation, full validation passed, and `apps/preview` now carries a repo-owned live repaint regression for engine-tab and appearance-only style mutations. Spec 071 is now **Closeout Ready** on `feat/071-preview-render-node-graph`: Phase 4 landed the no-central-branching proof in `packages/layout-engine/tests/preview-node-onboarding.test.ts`, which registers `dummy-onboarding-grid` through the public preview-engine registry, switches it through `commitPreviewSwitchNodeLayoutEngine(...)`, renders it through the shared render seam, and source-guards `preview-render-node.ts`, `preview-switch-node.ts`, `scripts/preview/editor.js`, and `scripts/preview/layout-bridge.js` against dummy-engine branching. `specs/071-preview-render-node-graph/evidence/render-path-inventory.md` is now a post-migration closeout inventory with one stage-mount owner (`preview-render-node.ts`) and one render-intent owner (`preview-switch-node.ts`), and `specs/071-preview-render-node-graph/evidence/validation-summary.md` records a full green validation run for `build:browser`, both package test suites, browser-bundle freshness, preview-shell size budgets, and `check_no_new_python`. Spec 062 is now **Closeout Ready** on `feat/062-parent-child-hug-resize-propagation`: `layout.ts` now remeasures constrained `HUG` leaves from the resized parent width instead of preserving authored fixed dimensions, `packages/layout-engine/tests/app-fresh-render.test.ts` proves the real `test-alignment-grid` fixture reflows after a `FIXED -> HUG` mutation plus a smaller parent width override, `apps/preview/src/persistence/frame-diagram.test.ts` covers the required `persist -> reload` round-trip, and `apps/preview/src/persistence/editor-hug-resize-regression.test.ts` provides the real-browser drag proof. `specs/062-parent-child-hug-resize-propagation/evidence/validation-summary.md` records the fixture facts, typed resize path, targeted regressions, and full green validation for `packages/layout-engine test`, `apps/preview test`, and `check_no_new_python`.
