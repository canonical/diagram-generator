...
- **Current handoff note:** spec 053 is merged to `main` and archived under `docs/spec-archive/053-preview-editor-post-refactor-correctness/`. Spec 054 is now **Closeout Ready** on `feat/054-preview-persistence-model-typescript`: `packages/layout-engine/src/preview-shell/preview-override-model.ts` owns canonical save-payload assembly for frame/arrow/grid/removal/engine-layout state, emitted keys are filtered through the shared `frame-override-manifest.ts`, `preview-arrow-component-ids.ts`, and `frame-yaml-engine-layout-contract.ts` owners, `app-save-payload.ts` is reduced to a guard/assertion layer, and repo-owned persist->reload coverage now explicitly covers canonical frame moves/resizes plus duplicate/explicit/branch arrow waypoint saves and removals. Full spec closeout validation passes on this branch. Do not continue that migration on `main` or on archived 053 paths.
