...
- **Active spec (when relevant):** Use `docs/specs.md` as the active spec index; `TODO.md` is only a pointer. `specs/065-interactive-relayout-contract/` remains the only active implementation spec and is blocked on the uncaptured historical `baseline-fail.json` requirement. Specs 046, 047, 048, 050, 051, 052, 053, 054-060, 062, 063, 066-069, 071, and 072 are merged to `main` and archived under `docs/spec-archive/`. Specs 073 and 074 are merged to `main` and ready for archive/branch cleanup. Preserve spec 046's ratchet: do not reopen it unless future work widens `scripts/preview/editor.js`, `scripts/preview/layout-bridge.js`, or reintroduces central preview-host/document-kind/engine branching.
...
- **Current handoff note:** The merged closeout cohort from the 2026-07-05 audit is archived under `docs/spec-archive/`: 048, 051, 052, 054-060, 062, 063, and 071 joined the already-archived 046, 047, 050, 053, 066-069, and 072 packages, and the catalog / handover docs no longer cite deleted feature branches for those slices. Spec 073 is merged to `main`: `grid` is normalized to canonical `frame` with a boundary alias, lane panels register through data contributions instead of the old central `PREVIEW_PANEL_REGISTRY`, force parameters flow through the shared layout-params controller while `force-spec` routes/persistence stay intact, the no-`algorithmFamily` guard is locked in tests, and the remaining host-template registration seam plus force host/route/persistence convergence are explicitly deferred to follow-up spec work. Validation was green on the branch at `packages/layout-engine` `981/981`, `apps/preview` `161/161`, `check_no_new_python` ok, and preview-shell size budgets ok. Spec 074 is also merged to `main`: Dagre is removed from the builtin registry/render surfaces and active build/preview tooling path, legacy Dagre saves canonicalize onto `elk-layered` / `meta.elk` with persist->reload coverage, every builtin preview engine declares a unique `algorithmClass`, and the adversarial review hardening reran green (`packages/layout-engine` `978/978`, `apps/preview` `160/160`, `check_no_new_python` ok). `INBOX.md` still carries the recurring raw notes by explicit user instruction, so closeout/admin cleanup there remains separate from merged product work. The queue now continues with 061 -> 064 -> 070, with 065 still blocked on the historical baseline artifact.
