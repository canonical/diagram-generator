...
- **Active spec (when relevant):** Use `docs/specs.md` as the active spec index; `TODO.md` is only a pointer. `specs/073-layout-node-model-param-unification/` is the current implementation slice, and `specs/065-interactive-relayout-contract/` remains active but blocked on the uncaptured historical `baseline-fail.json` requirement. Specs 046, 047, 048, 050, 051, 052, 053, 054-060, 062, 063, 066-069, 071, and 072 are merged to `main` and archived under `docs/spec-archive/`. Preserve spec 046's ratchet: do not reopen it unless future work widens `scripts/preview/editor.js`, `scripts/preview/layout-bridge.js`, or reintroduces central preview-host/document-kind/engine branching. Spec 074 remains **Closeout Ready** on `feat/074-layout-algorithm-consolidation`; its review hardening and closeout-doc reconciliation are complete, but the package is not archived yet.
...
- **Current handoff note:** The merged closeout cohort from the 2026-07-05 audit is now archived under `docs/spec-archive/`: 048, 051, 052, 054-060, 062, 063, and 071 joined the already-archived 046, 047, 050, 053, 066-069, and 072 packages, and the spec catalog / handover docs no longer cite deleted feature branches for those slices. Spec 074 remains **Closeout Ready** on `feat/074-layout-algorithm-consolidation`: the decision matrix and downstream-spec queue are committed, Dagre is removed from the builtin registry/render surfaces and active build/preview tooling path, legacy Dagre saves canonicalize onto `elk-layered` / `meta.elk` with persist->reload coverage, every builtin preview engine declares an `algorithmClass`, the duplicate-algorithm guard is enforced in the registry plus repo-owned tests, and the adversarial review hardening reran green (`packages/layout-engine` `978/978`, `apps/preview` `160/160`, `check_no_new_python` ok). `INBOX.md` still carries the recurring raw notes by explicit user instruction, so closeout/admin cleanup there remains separate from merged product work. The priority queue continues with spec 073, then 061 -> 064 -> 070.
