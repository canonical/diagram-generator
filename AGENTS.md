...
- **Current handoff note:** specs 053-056 are now merged to `main`. Spec 054 moved preview save-payload ownership into typed TS owners: `packages/layout-engine/src/preview-shell/preview-override-model.ts` emits canonical frame/arrow/grid/removal/engine-layout state, emitted keys are filtered through `frame-override-manifest.ts`, `preview-arrow-component-ids.ts`, and `frame-yaml-engine-layout-contract.ts`, and `app-save-payload.ts` is reduced to a guard/assertion layer with persist->reload regression coverage for frame moves/resizes plus duplicate/explicit/branch arrow waypoint saves and removals. Spec 055 added `preview-engine-workspace.ts` and `preview-engine-workspace-chrome.ts` as the typed engine-tab workspace owner, kept `scripts/preview/engine-switcher.js` as a thin bootstrap shim, and updated preview-host/runtime wiring so active engine identity, reopen semantics, and save semantics stay coherent across engine tabs. Spec 056 added the typed reroute invalidation path: `preview-arrow-reroute-invalidation.ts`, shared reroute invalidation keys in `frame-override-manifest.ts`, reroute clearing in relayout/fresh-render lanes, and save-path waypoint clearing that preserves explicit arrow ids and `arrow:<id>` attachment targets across reload. Spec 057 is now implemented in this worktree: preview-engine compatibility distinguishes offer-list example fit from explicit engine resolution, `elk-rectpacking` is no longer offered for authored process-flow examples, ELK-family engines now reject fill-sized structural carriers that lack an authored `meta.diagram_type`, nested annotation restoration in `packages/layout-engine/src/elk-layout.ts` keeps `mongo-octavia-ha` AZ labels attached to their wrappers, and preview-engine workspace state now keeps an explicitly resolved active engine visible even when it is no longer in the offer list. Full validation is green on the current code.
