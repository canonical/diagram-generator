...
- **Current handoff note:** specs 053-060 have product closeout complete. Spec 066 is merged to `main` and archived; spec 067 remains archived as a folded review artifact. Specs 068 and 069 are merged to `main`, and spec 069 is archived under `docs/spec-archive/069-editor-mutation-state-determinism/`: all tasks are complete, adversarial review gaps were addressed, browser evidence is `ok: true`, SC-001 probe coverage is hash-guarded in spec validation, full validation passed, and `apps/preview` now carries a repo-owned live repaint regression for engine-tab and appearance-only style mutations. Spec 071 is active on `feat/071-preview-render-node-graph`: Phases 1 and 2 are complete and committed (T021 `9b08d21`, T022 `69e68fc`), the Phase 1 hardening follow-ups T016/T017 are committed in `be96d32`, the Phase 2 save/reload follow-ups T023/T024 are now complete in the current tree, and the Phase 3 switch/cook slice (T030/T031/T032) is implemented with its review note under `specs/071-preview-render-node-graph/evidence/phase-3-adversarial-review.md`. The new save contract treats `meta.<family>_nodes` as a full replacement, empty node buckets clear interpreter params instead of persisting `{}`, and `editor-live-repaint-regression.test.ts` now proves both non-active bucket survival across save→reload and deletion after a later full clear. Remaining open work is Phase 4 onboarding/inventory/closeout work (T040+).
