...
- **Active spec (when relevant):** `docs/spec-archive/044-preview-shell-architecture-followup/` and `docs/spec-archive/045-preview-host-engine-modularity/` are now merged and archived on `main`. `packages/layout-engine/src/browser-entry.ts` exposes only `core`, `previewBridge`, `previewEngines`, and `previewShell`; the remaining legacy browser wrappers in `scripts/preview/` resolve only those namespaced owners; and preview-host lane/page assembly stays in typed `preview-host/` owners rather than `server.ts`. `specs/046-editor-host-endgame/` remains architecturally closed; do not reopen it unless future work widens `scripts/preview/editor.js`, `scripts/preview/layout-bridge.js`, or reintroduces central preview-host/document-kind/engine branching. `specs/047-render-ir-unification/` is the next architecture package and should proceed only on a matching `feat/047-render-ir-unification` branch. Use `docs/specs.md` as the active spec index; `TODO.md` is only a pointer.
