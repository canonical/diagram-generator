...
- **Current handoff note:** specs 053-060 have product closeout complete. Spec 066 is merged to `main` and archived; spec 067 remains archived as a folded review artifact. Specs 068 and 069 are merged to `main`, and spec 069 is archived under `docs/spec-archive/069-editor-mutation-state-determinism/`: all tasks are complete, adversarial review gaps were addressed, browser evidence is `ok: true`, SC-001 probe coverage is hash-guarded in spec validation, full validation passed, and `apps/preview` now carries a repo-owned live repaint regression for engine-tab and appearance-only style mutations. Spec 071 is active on `feat/071-preview-render-node-graph`: the branch-local Phase 0 canvas-parity baseline is captured under `specs/071-preview-render-node-graph/evidence/canvas-parity-baseline.json`, Phase 2 has unit + browser proof that layered, radial, and dagre layout buckets stay isolated in-session, and layout-parameter ownership no longer lives on `layoutOperatorOverrides` / `__DG_activeLayoutOperatorKey`; remaining open work is per-node persistence and the switch/cook phases.
