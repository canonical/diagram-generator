...
- **Active spec (when relevant):** Use `docs/specs.md` as the active spec index; `TODO.md` is only a pointer. `specs/051-preview-editor-contextual-aside/` drafts the UI cleanup plan for engine/selection-driven right-aside visibility. `specs/052-layers-palette-reorder/` drafts same-parent drag and keyboard reorder in the Layers palette. `specs/047-render-ir-unification/` remains **Closeout Ready** on `feat/047-render-ir-unification`. `specs/048-elk-sizing-interaction-followup/` is **Closeout Ready**. `specs/046-editor-host-endgame/` remains architecturally closed; do not reopen it unless future work widens `scripts/preview/editor.js`, `scripts/preview/layout-bridge.js`, or reintroduces central preview-host/document-kind/engine branching. `specs/050-preview-editor-recovery/` was merged to `main` and archived under `docs/spec-archive/050-preview-editor-recovery/`.
...
- **Current handoff note:** spec 050 is merged and archived. Specs 051 and 052 are draft planning packages only; implementation should happen on their matching feature branches and in typed preview-shell or preview-engine owners, not by widening `scripts/preview/editor.js` or `scripts/preview/layout-bridge.js`.
