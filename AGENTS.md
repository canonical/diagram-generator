...
- **Active spec (when relevant):** Use `docs/specs.md` as the active spec index; `TODO.md` is only a pointer. `specs/073-layout-node-model-param-unification/` is the current implementation slice. `specs/065-interactive-relayout-contract/` remains active but blocked on the uncaptured historical `baseline-fail.json` requirement. Spec 046 is merged to `main` and archived under `docs/spec-archive/046-editor-host-endgame/`; preserve its ratchet and do not reopen it unless future work widens `scripts/preview/editor.js`, `scripts/preview/layout-bridge.js`, or reintroduces central preview-host/document-kind/engine branching. Spec 047 was merged to `main` as fast-forward `e7b3f65` on 2026-06-23 and is archived under `docs/spec-archive/047-render-ir-unification/`. `specs/072-preview-engine-hardening/` is merged to `main` and archived under `docs/spec-archive/072-preview-engine-hardening/`. `specs/068-internal-dual-path-deletion/` and `specs/069-editor-mutation-state-determinism/` were merged to `main`, with spec 069 archived under `docs/spec-archive/069-editor-mutation-state-determinism/`. `specs/054-preview-persistence-model-typescript/`, `specs/055-preview-engine-workspace-navigation/`, `specs/056-arrow-reroute-structural-mutations/`, `specs/057-graph-engine-fidelity-and-example-fit/`, `specs/058-layer-tree-inspector-selection-ergonomics/`, and `specs/059-cross-document-style-source-of-truth/` are **Closeout Ready** and merged or ready on their feature branches; archive merged packages after branch cleanup. `specs/060-output-pane-engine-tabs-rerender/` remains **Closeout Ready** with a follow-up note for visually no-op engine switches. `specs/052-layout-engine-onboarding-port/` is **Closeout Ready** on `feat/052-layout-engine-onboarding-port`. `specs/051-preview-editor-contextual-aside/` is **Closeout Ready** and was live reverified on 2026-06-29. `specs/048-elk-sizing-interaction-followup/` is **Closeout Ready**. `specs/053-preview-editor-post-refactor-correctness/` was merged to `main` and archived under `docs/spec-archive/053-preview-editor-post-refactor-correctness/`. `specs/050-preview-editor-recovery/` was merged to `main` and archived under `docs/spec-archive/050-preview-editor-recovery/`. `specs/063-auto-style-by-nesting-depth/`, `specs/066-graph-engine-layout-option-surfacing/`, and `specs/067-layout-engine-parameter-pane/` are archived under `docs/spec-archive/`.
...
- **Current handoff note:** specs 053-060 have product closeout complete. Spec 066 is merged to `main` and archived; spec 067 remains archived as a folded review artifact. Specs 068 and 069 are merged to `main`, and spec 069 is archived under `docs/spec-archive/069-editor-mutation-state-determinism/`: all tasks are complete, adversarial review gaps were addressed, browser evidence is `ok: true`, SC-001 probe coverage is hash-guarded in spec validation, full validation passed, and `apps/preview` now carries a repo-owned live repaint regression for engine-tab and appearance-only style mutations. Spec 071 remains **Closeout Ready** on `feat/071-preview-render-node-graph` with the no-central-branching onboarding proof, post-migration render-path inventory, and full green validation summary committed. Spec 062 is back to **Closeout Ready** on `feat/062-parent-child-hug-resize-propagation` after the review-hardening rerun revalidated the nested non-leaf `HUG` fix and the unconstrained root/top-level `HUG` follow-on fix. Spec 063 remains **Closeout Ready** on `feat/063-auto-style-by-nesting-depth`. Spec 072 is merged to `main` and archived under `docs/spec-archive/072-preview-engine-hardening/`: the render-node fit seam is structurally guarded, the stale active-engine badge path is removed, section-heading bottom spacing is increased through the shared style contract, browser persistence/padding proofs are live, and Playwright-backed preview regressions now skip cleanly when chromium is unavailable. Validation was green at merge (`packages/layout-engine` `973/973`, `apps/preview` `160/160`, `check_no_new_python` ok). Spec 074 remains **Closeout Ready** on `feat/074-layout-algorithm-consolidation`: the decision matrix and downstream-spec queue are committed, Dagre is removed from the builtin registry/render surfaces and the active build/preview tooling path, legacy Dagre saves canonicalize onto `elk-layered` / `meta.elk` with persist->reload coverage, every builtin preview engine declares an `algorithmClass`, the duplicate-algorithm guard is enforced in the registry plus repo-owned tests, the adversarial review hardening is reconciled (runtime blank-value rejection without manifest-identity regression plus orphaned `meta.dagre*` canonicalization), and the full rerun stayed green (`packages/layout-engine` `978/978`, `apps/preview` `160/160`, `check_no_new_python` ok). `INBOX.md` still carries the recurring raw notes by explicit user instruction, so closeout/admin cleanup there remains separate from the merged product work. The priority queue now continues with spec 073, then 061 -> 064 -> 070.
