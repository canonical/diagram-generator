...
- **Current handoff note:** specs 053-060 have product closeout complete. Spec 066 is merged to `main` and archived; spec 067 remains archived as a folded review artifact. Specs 068 and 069 are merged to `main`, and spec 069 is archived under `docs/spec-archive/069-editor-mutation-state-determinism/`: all tasks are complete, adversarial review gaps were addressed, browser evidence is `ok: true`, SC-001 probe coverage is hash-guarded in spec validation, full validation passed, and `apps/preview` now carries a repo-owned live repaint regression for engine-tab and appearance-only style mutations. Spec 071 is active on `feat/071-preview-render-node-graph` (HEAD `69e68fc` before the current working slice): Phases 1 and 2 are complete and committed (T021 `9b08d21`, T022 `69e68fc`), both were adversarially reviewed on 2026-07-02, and the Phase 1 hardening follow-ups T016/T017 are now implemented in the working tree — findings and follow-up inventory live in `specs/071-preview-render-node-graph/evidence/adversarial-review.md`. Remaining open work is the Phase 2 correctness/browser follow-ups (T023/T024) plus the Phase 3 switch/cook phases (T030+).
