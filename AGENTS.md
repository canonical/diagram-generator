...
- **Current handoff note:** spec 053 is closeout-ready on `feat/053-preview-editor-post-refactor-correctness`: alignment, direction/stage resize, explicit v3 engine display, compatibility-filtered engine lists, and active-manifest engine controls are implemented. Post-pull validation on 2026-06-26 refreshed two test contracts on this branch: YAML persistence expectations in `apps/preview/src/persistence/frame-diagram.test.ts` now normalize LF/CRLF, and the single-engine v3 preview-host fixture now uses a valid no-arrow frame with page direction so the disabled populated engine field renders. A later same-day follow-up fixed the remaining live inspector regressions: the single-selection variant picker renders again, autolayout leaf alignment targets the effective parent container, and headed-container relayout now mirrors `align` overrides onto the synthetic `__body` node. The same-day persistence hardening pass now canonicalizes transient frame deltas before save, drops synthetic wrapper ids, preserves typed engine-control values, separates reload failures from persist failures, and resolves duplicate arrow ids over the authored arrow sequence. A later architectural follow-up fixed a deeper preview-routing identity bug: `routePreviewArrows` now keeps authored arrow ids intact for `arrow:<id>` / `@id` attachment routing while carrying preview component ids separately for selection/save. Draft spec 054 tracks the remaining owner migration out of `scripts/preview/component-model.js`. Live DOM verification against `http://127.0.0.1:8101/view/v3:test-alignment-grid` confirmed `small_box` moved from roughly `(439,259)` to `(550,370)` on `BOTTOM_RIGHT`, and variant `parent` changed fill/stroke to `#F3F3F3`. Validation is green: focused spec 053 inspector/relayout tests plus `build:browser`. The in-app browser backend remained unavailable, so browser verification used a headless system Chrome DOM probe.
