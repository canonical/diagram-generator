...
- **Current handoff note:** spec 053 is merged to `main` and archived under `docs/spec-archive/053-preview-editor-post-refactor-correctness/`. Spec 054 is now **In Progress** on `feat/054-preview-persistence-model-typescript`: `packages/layout-engine/src/preview-shell/preview-override-model.ts` owns typed save-payload assembly for overrides/grid/removals/engine-layout state, `app-save-client.ts` builds the normal save payload from that typed owner before guard normalization, and `scripts/preview/component-model.js::toOverridePayload()` is reduced to a namespaced bootstrap delegation wrapper. T011 is now landed: emitted frame/arrow/engine-layout keys are filtered through the shared `frame-override-manifest.ts`, `preview-arrow-component-ids.ts`, and `frame-yaml-engine-layout-contract.ts` owners before preview persistence hands off to the app save path. The remaining 054 work is to demote `app-save-payload.ts` to a guard-only layer, widen save round-trip coverage, and finish the documentation/closeout-gate updates. Do not continue that migration on `main` or on archived 053 paths.
