...
- **Active spec (when relevant):** Use `docs/specs.md` as the active spec index; `TODO.md` is only a pointer. `specs/065-interactive-relayout-contract/` remains blocked on the uncaptured historical `baseline-fail.json` requirement, and `specs/076-tls-mermaid-cold-start-fit/` is now **Closeout Ready** on `feat/076-tls-mermaid-cold-start-fit`. Specs 046-048, 050-060, 062-063, 066-074 are merged to `main` and archived under `docs/spec-archive/`. Preserve spec 046's ratchet: do not reopen it unless future work widens `scripts/preview/editor.js`, `scripts/preview/layout-bridge.js`, or reintroduces central preview-host/document-kind/engine branching.
...
- **Current handoff note:** The merged closeout cohort from the 2026-07-05 audit is archived under `docs/spec-archive/`: 048, 051, 052, 054-060, 062, 063, 071, 073, and 074 now join the already-archived 046, 047, 050, 053, 066-069, and 072 packages, and the catalog / handover docs no longer cite deleted feature branches for those slices. Spec 073 archived with the canonical `frame` shell rename, registration-driven lane panels, shared force layout-params integration, and the no-`algorithmFamily` guard; the remaining host-template registration seam and force host/route/persistence convergence stay explicitly deferred to follow-up spec work. Spec 074 archived with Dagre retired from runtime and active build/preview tooling, legacy Dagre saves canonicalized onto `elk-layered` / `meta.elk`, and the hard unique-`algorithmClass` guard enforced in the preview-engine registry. Validation for the merged mainline is green at `packages/layout-engine` `985/985`, `apps/preview` `161/161`, `build:browser` pass, `check-browser-bundle-fresh` ok, `check_no_new_python` ok, and preview-shell size budgets ok. `INBOX.md` still carries the recurring raw notes by explicit user instruction, so closeout/admin cleanup there remains separate from merged product work. The queue now continues with 061 -> 064 -> 070, with 065 still blocked on the historical baseline artifact.
- **Local 076 branch note:** `feat/076-tls-mermaid-cold-start-fit` is now closeout-ready. The bounded ELK-only follow-up landed as a generic typed lowering shim: authored/native compounds keep local direction through `GraphNodeInput.direction`, invisible ordering edges preserve Mermaid-style row order without relying on the hierarchy+model-order ELK option set that crashes, and ordering edges stay layout-only / filtered from rendered routes. The TLS fixture now declares `deployment_and_runtime_topology` + `elk-layered`, the sibling-promotion levels are corrected, and fixture-owned compatibility + geometry regressions prove `openstack_relation_row` stays above `octavia_k8s` while the endpoint row stays `traefik_public`, `traefik_internal`, `traefik_rgw`. Existing Mongo availability-zone fidelity remains green after fixing a dead locked-container overflow branch in `wrapStructuralContainers(...)`. Validation is green: `packages/graph-layout-elk` 44/44, `packages/layout-engine` 992/992, `apps/preview` 160 pass / 6 skip (`Playwright chromium unavailable`), `check_no_new_python`, `build:browser`, and `check-browser-bundle-fresh`.
