...
- **Active spec (when relevant):** `docs/spec-archive/044-preview-shell-architecture-followup/` and `docs/spec-archive/045-preview-host-engine-modularity/` are merged and archived on `main`. `specs/047-render-ir-unification/` is now active on `feat/047-render-ir-unification`: the first live slice extracts shared frame-arrowhead geometry into a neutral TypeScript helper, removes the preview-engine -> preview-shell ownership leak in ELK debug rendering, and hardens parity coverage so arrowheads and short-segment shaft truncation are compared across legacy SVG and display-list serializers. `specs/046-editor-host-endgame/` remains architecturally closed; do not reopen it unless future work widens `scripts/preview/editor.js`, `scripts/preview/layout-bridge.js`, or reintroduces central preview-host/document-kind/engine branching. Use `docs/specs.md` as the active spec index; `TODO.md` is only a pointer.
