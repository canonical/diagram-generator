...
- **Active spec (when relevant):** Use `docs/specs.md` as the active spec index; `TODO.md` is only a pointer. `specs/065-interactive-relayout-contract/` remains the only active implementation spec and is blocked on the uncaptured historical `baseline-fail.json` requirement. Specs 046-048, 050-060, 062-063, 066-074 are merged to `main` and archived under `docs/spec-archive/`. Preserve spec 046's ratchet: do not reopen it unless future work widens `scripts/preview/editor.js`, `scripts/preview/layout-bridge.js`, or reintroduces central preview-host/document-kind/engine branching.
...
- **Local 076 branch note:** `feat/076-tls-mermaid-cold-start-fit` executed the spec 076 T0 gate on 2026-07-06. `tmp/elk-cluster-spike.mts` and the corrected sibling Mermaid ELK oracle (`tmp/mermaid-tls-elk-restyled.svg` / `.png`, rendered via `../mermaid/restyle.mjs --export-only`) both preserve compounds but fail the reference-critical lower-cluster stacking; the hand-authored spike also reorders the endpoint row. Opus reviewed the evidence on 2026-07-07 and rejected the Dagre pivot: the spike never enabled ELK ordering, the dagre near-match requires a fabricated helper edge, and the real blocker is the uninvestigated `elkjs` crash under hierarchy + model-order. Do not continue Phase 2; the next step is the bounded ELK ordering experiment from `docs/spec-reviews/076-tls-mermaid-cold-start-fit.md`.
