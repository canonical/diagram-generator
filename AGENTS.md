...
- **Active spec (when relevant):** `specs/046-editor-host-endgame/` now has its closeout bar met on `feat/046-editor-host-endgame`. On this branch the honest answer is now **yes** for adding **50, 150, and 500 heterogeneous layout engines** without widening `editor.js`, `layout-bridge.js`, or central preview-host/document-kind sinks. Adjacent follow-on work in `specs/044-preview-shell-architecture-followup/`, `specs/045-preview-host-engine-modularity/`, and `specs/047-render-ir-unification/` should stay independent and should not reopen 046 unless they reintroduce behavior-heavy legacy JS ownership or ELK-only substrate contracts. See `AGENT-INBOX.md`, `TODO.md`, and `docs/specs.md`.
