...
- **Current handoff note:** specs 053-057 and 060 are now merged to `main`. Spec 058 is under active closeout review on `feat/058-layer-tree-inspector-selection-ergonomics`; verify the typed layer-tree DOM keyboard contract and the `test-deep-nesting` effective-variant path before marking it ready. Spec 059 has no product work yet and should remain Draft until started on its matching branch.
