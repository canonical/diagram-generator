...
- **Current handoff note:** specs 053-060 have product closeout complete. Spec 066 is merged to `main` and archived; spec 067 remains archived as a folded review artifact. Specs 068 and 069 are merged to `main`, and spec 069 is archived under `docs/spec-archive/069-editor-mutation-state-determinism/`: all tasks are complete, adversarial review gaps were addressed, browser evidence is `ok: true`, SC-001 probe coverage is hash-guarded in spec validation, full validation passed, and `apps/preview` now carries a repo-owned live repaint regression for engine-tab and appearance-only style mutations. Spec 071 is **Closeout Ready** on `feat/071-preview-render-node-graph` with the no-central-branching onboarding proof, post-migration render-path inventory, and full green validation summary committed. Spec 062 is back to **Closeout Ready** on `feat/062-parent-child-hug-resize-propagation` after the review-hardening rerun revalidated the nested non-leaf `HUG` fix and the unconstrained root/top-level `HUG` follow-on fix. Spec 063 is **Closeout Ready** on `feat/063-auto-style-by-nesting-depth` and committed at `8e58b81`. Spec 072 is now active on `feat/072-preview-engine-hardening`: it triages the remaining honest-open inbox issues into a dedicated package, hardens the render-node fit/mount order, restores direct browser proof that right/bottom canvas padding survives `Autolayout -> ELK layered`, widens the no-central-branching guard to cover `v3` and `sequence`, routes standalone preview-document kinds through typed helpers instead of central `sequence` branches, replaces the builtin install-unit call ladder with a shared builtin collection, and extends the browser persistence proof to verify live save→reload→switch-back state for layered/radial/dagre. Remaining open 072 work is the stale active-engine badge removal and the section-heading bottom-spacing increase. The Opus-ordered queue in `TODO.md` remains 061 -> 064 -> 072.
