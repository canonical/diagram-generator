...
- **Current handoff note:** specs 053-057 and 060 are now merged to `main`; spec 058 is closeout-ready on `feat/058-layer-tree-inspector-selection-ergonomics`. Spec 058 keeps ownership in typed preview-shell owners, adds fixture-backed `test-deep-nesting` effective-variant coverage, and records browser evidence for real-DOM layer-tree traversal plus the `vm_2` inspector variant. Spec 059 has no product work yet and should remain Draft until started on its matching branch.
