...
export function buildGridInfo(diagram: FrameDiagram, root: Frame): GridInfo {
  ...
  const lastColW =
    cols > 1 ? Math.max(colW, contentW - (cols - 1) * (colW + colGap)) : contentW;
  ...
  let rowCount = 1;
  let rowH =
    contentH > 0 ? Math.floor(contentH / BASELINE_UNIT) * BASELINE_UNIT : 0;
  if (rowH > 0) {
    const targetRowH = Math.max(BASELINE_UNIT * 10, 80);
    rowCount = Math.max(
      1,
      Math.floor((contentH + rowGapSnapped) / (targetRowH + rowGapSnapped)),
    );
