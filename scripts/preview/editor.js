...
document.getElementById("btn-export-png").addEventListener("click", async () => {
  const svgEl = document.querySelector("#stage svg");
  if (!svgEl) { alert("No diagram loaded."); return; }
  const clone = svgEl.cloneNode(true);
  // Strip editor-only overlays (grid, guides, handles, selection)
  clone.querySelectorAll("#layout-grid, .dg-guide, .dg-handle, .dg-selection").forEach(el => el.remove());

  // Embed @font-face so the SVG-in-img has access to the font
  try {
    const fontUrls = [];
    for (const sheet of document.styleSheets) {
      try {
        for (const rule of sheet.cssRules) {
          if (rule instanceof CSSFontFaceRule && rule.style.fontFamily.replace(/['"]/g, "").includes("Ubuntu Sans")) {
            const src = rule.style.getPropertyValue("src");
            const urlMatch = src.match(/url\(["']?([^"')]+)["']?\)/);
            if (urlMatch) fontUrls.push({ url: urlMatch[1], rule: rule.cssText });
          }
        }
    ...
    if (fontUrls.length > 0) {
      ...
      const parts = [];
      for (const { url, rule } of fontUrls) {
        try {
          const resp = await fetch(url);
          const buf = await resp.arrayBuffer();
          const b64 = btoa(String.fromCharCode(...new Uint8Array(buf)));
          const mime = url.endsWith(".woff2") ? "font/woff2" : url.endsWith(".woff") ? "font/woff" : "font/ttf";
          parts.push(rule.replace(/url\(["']?[^"')]+["']?\)/, "url(data:" + mime + ";base64," + b64 + ")"));
        } catch (_) { parts.push(rule); }
      }
      styleEl.textContent = parts.join("\n");
      clone.insertBefore(styleEl, clone.firstChild);
    }
  } catch (_) { /* proceed without embedded fonts */ }

  const w = parseFloat(clone.getAttribute("width") || clone.viewBox.baseVal.width || 800);
