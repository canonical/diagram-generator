...
Re-reviewed on 2026-06-14 after the native-ELK cleanup, headed-compound fix, and compatibility sweep. The implementation now matches the tightened spec contract: midpoint ports are modeled as ELK inputs, route geometry and edge-label placement stay ELK-authored, and headed/icon-bearing groups remain ELK-compatible because their chrome is treated as decorative container padding rather than as routing participants. Carriers with bidirectional external descendant traffic are flattened back into the ELK input when native compound boundaries would force detour-heavy routes.
...
- `complex-routing-usecase.yaml` had been treated as non-native only because headed authored groups were flattened out of the ELK graph and then wrapped again afterward. `packages/layout-engine/src/elk-layout.ts` now restores native compound treatment for headed carriers whose descendant traffic across the boundary is one-way, while still flattening bidirectional carriers such as `juju_system` that would otherwise force ELK to route around the boundary.
...
- Headings and icons on compounds were re-checked as decorative chrome only: they move with the wrapper box, remain outside the ELK child-participant set, and no longer disqualify the diagram from ELK.
- The real failing preview corpus, `juju-bootstrap-machines-process.yaml`, was re-checked from the routed data:
  ...
  - `step3` is now a direct vertical segment in the preview layout instead of a boundary-detour path
...
- `complex-routing-usecase.yaml` was re-checked from the laid-out data:
  - `planning`, `implementation`, and `delivery` remain native ELK compounds because their external descendant traffic is one-way
