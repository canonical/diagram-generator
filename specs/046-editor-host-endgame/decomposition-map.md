...
- `scripts/preview/editor.js`: 1,601 physical lines in the current working tree
  on the active 046 branch
- `scripts/preview/layout-bridge.js`: 395 physical lines in the current working
  tree on the active 046 branch
- Closeout status: **still open**. The integration-sink risk is materially
  lower than it was, but the honest answer to the 50/150/500-engine question is
  still "not yet" because both legacy JS files still remain too
  coordinator-shaped and the final install-proof plus adversarial audit work is
  still pending.
