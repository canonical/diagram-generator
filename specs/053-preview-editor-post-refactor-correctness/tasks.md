...
- [x] **T000** Confirm branch and working tree scope.
...
- [x] **T001** Capture failing alignment behavior.
...
- [x] **T002** Capture failing v3 direction/stage resize behavior.
...
- [x] **T003** Capture failing engine UI behavior.
...
- [x] **T010** Fix single-frame alignment mutation.
...
- [x] **T011** Preserve alignment through save/reload.
...
- [x] **T012** Separate parent alignment from multi-selection align.
...
- [x] **T013** Resize stage/canvas after v3 direction changes.
...
- [x] **T020** Make implicit v3 explicit in preview config.
...
- [x] **T021** Filter compatible engines by registry result.
...
- [x] **T022** Render active engine controls, not layered defaults.
...
- [x] **T023** Keep control persistence namespace-safe.
...
- [x] **T031** Live no-screenshot DOM probe.
...
- Alignment and direction mutations now request immediate typed relayout instead
  of delayed text relayout, covering the stale-stage symptoms for visual layout
  properties.
- The engine field is populated for implicit v3 pages, including single-engine
  frame diagrams where the switcher is visible but disabled.
- Explicit unresolved/non-layered engines no longer fall back to stale
  `elk-layered` controls, and no-arrow frame documents expose only `v3` plus a
  clear save rejection for graph engines.
- Validation run on 2026-06-26: `npm --prefix packages/layout-engine test`;
  `npm --prefix apps/preview test`; `npm --prefix packages/layout-engine run
  build`; `npm --prefix packages/layout-engine run build:browser`; `node
  scripts/check-browser-bundle-fresh.mjs`; `node
  scripts/check_no_new_python.mjs`; local text probe against
  `http://127.0.0.1:8123/view/test-alignment-grid` and
