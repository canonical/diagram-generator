...
| Drag, resize, keyboard nudge, delete, undo/redo | `preview-shell/interaction/*`, `preview-shell/app-bootstrap.ts` | Existing contract tests + live probe (2026-06-24) on `preview-smoke` and `mongo-octavia-ha` | **Partially verified.** Delete + Undo + Redo round-trip live with faithful keyboard/mouse input: leaf delete removes the node (15→14), the active toolbar Undo enables + Save flips `dirty`, Ctrl+Z restores (14→15) and Redo enables. Keyboard nudge (ArrowRight) is correctly inert on autolayout children (Position: Auto). DOM note: two Undo/Redo/Save button sets exist (editor toolbar + foundry chrome); the active set carries the `dirty` Save class. | Live-probe drag-reorder, handle resize, live-resize, and nudge on an absolutely-positioned frame (T013). |
