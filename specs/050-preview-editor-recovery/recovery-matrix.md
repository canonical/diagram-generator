...
| Diagram picker next/previous and browse links | `preview-shell/app-diagram-navigation.ts` | Unit tests for picker sync, stepped navigation, browse active state + live probe (2026-06-24) | **Pass.** Live on `preview-smoke`: the `Next diagram` button navigates to the adjacent slug (`preview-smoke` \u2192 `request-to-hardware-stack`), and driving the picker `<select>` change navigates to the next slug (\u2192 `service-handshake-sequence`). `← all previews` browse link and stepping callbacks are unit-covered by `initPreviewDiagramNavigation`. (Note: the next/prev buttons sit at the far right of the toolbar, so a faithful click needs a wide viewport.) | Resolved. |
