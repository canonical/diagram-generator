...
- [x] T020 Refactor fresh preview render to consume the shared render IR instead of hand-rolled frame/arrow DOM geometry
- [x] T021 Define or implement a preview DOM serializer that consumes the same IR as export
- [x] T022 Make `svg-render.ts` a thin wrapper over the shared IR path or retire it after parity holds

## Phase 4 - Duplicate renderer retirement

- [x] T030 Retire or sharply reduce duplicate bridge arrow render builders in `layout-bridge.js`
- [x] T031 Keep interactive arrow editing as a DOM mutation layer but route its geometry through shared primitives only
- [x] T032 Re-run export/preview parity checks and validate logical export grouping stays intact
